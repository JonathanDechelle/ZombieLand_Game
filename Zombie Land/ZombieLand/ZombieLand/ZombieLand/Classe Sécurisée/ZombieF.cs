...
namespace ZombieLand
{
    class ZombieF
    {
        ...
        public SpriteEffects Flip { get { return flip; } }
         ...
         public ZombieF(Vector2 Position)
        {
            this.Position = Position;
            RecZombie = new Rectangle((int)Position.X+40, (int)Position.Y, 80, 20);
            APZ = new AnimationPlayer();
            Walking = new Animation(Ressources.ZombieF, 80, 0.5f, 1, true);
            Dead = new Animation(Ressources.ZombieFDead, 80, 0.1f, 1, false);
         ...
         public void Update(ZLPlayer Joueur, List<ObjCollisionable> ObjCollisionnable)
         {
             ...
             else if (recPerso.Intersects(RecZombie))
             {
                 ...
                 { if (flip == SpriteEffects.None)Position.X++; else Position.X--; nbTouche++; if (nbTouche == 20)Killed = true; }
             ...
             if (Joueur.cartouches != null)
             {
                 for (int C = 0; C < Joueur.cartouches.Count; C++)
                 {
                     ...
                     if (nbTouche >= 10) { RecZombie = new Rectangle(-100, -100, 10, 10); ; Killed = true; }
                 }
             }
             #endregion
             ...
             if (Killed && !SoundOneTime) { SoundOneTime = true; Ressources.ZombieDie.Play(0.1f, -1f, 0f); }
         }
